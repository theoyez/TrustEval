    ...
    Promise.all([get(`../data/metrics/${L.value}.json`), get(`../data/metrics/${R.value}.json`)]).then(([l,r])=>{
      document.getElementById('cmp').innerHTML=`
        ...
        <table class="table zebra">
          ...
          <tbody>
            <tr><td>Accuracy</td><td class="right">${pct(l.summary.accuracy)}</td><td class="right">${pct(r.summary.accuracy)}</td><td>${badge((r.summary.accuracy - l.summary.accuracy)*100)}</td></tr>
